...
![图 2.1：转角函数从 0 平滑增加到 1，因而切向量从向右连续转到向左。](figures/classical-mcf-double-plane-neck/step-1-angle.svg)
...
![图 2.2：积分切向方向得到位于右半平面的 U 形母线。](figures/classical-mcf-double-plane-neck/step-2-template.svg)
...
![图 2.3：固定模板先按固定高度 h 缩放，再平移到半径 2R_i。](figures/classical-mcf-double-plane-neck/step-3-scale-translate.svg)
...
![图 2.4：两张近平面盘、缓慢过渡区和远处固定 U 形颈拼成完整母线。](figures/classical-mcf-double-plane-neck/step-4-assemble.svg)
...
![图 2.5：母线绕 z 轴旋转为闭合嵌入曲面。](figures/classical-mcf-double-plane-neck/step-5-revolve.svg)

*图 2.5　旋转不会制造重数：对每个固定 \(i\)，得到的仍是一张光滑嵌入球面；重数二只在取 \(i\to\infty\) 的局部极限时出现。*
...
![图 4.1：固定窗口内两张嵌入图重合，而连接颈逃向无穷远。](figures/classical-mcf-double-plane-neck/step-6-limit.svg)
