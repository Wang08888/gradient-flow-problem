...
固定 \(h>0\)。先选光滑非降函数 \(\eta_0:[0,1]\to[0,1]\)，使它在 \(0\)、\(1\) 的邻域分别恒为 \(0\)、\(1\)，并满足
\[
\eta_0(1-s)=1-\eta_0(s).
\]
令
\[
J=\int_0^1\sin(\pi\eta_0(s))\,ds>0,
\qquad L=\frac2J,
\]
并定义
\[
U(s)=L\int_0^s\cos(\pi\eta_0(v))\,dv,
\qquad
V(s)=1-L\int_0^s\sin(\pi\eta_0(v))\,dv.
\tag{2.0}
\]
由对称性，\(U(0)=U(1)=0\)、\(V(0)=1\)、\(V(1)=-1\)。曲线 \((U,V)\) 的速度恒为 \(L\)，其切向角从 \(0\) 单调转到 \(-\pi\)；\(U\) 先增后减且内部为正，\(V\) 单调下降。因此它是一条简单的右向 U 形弧。又因 \(\eta_0\) 在端点邻域恒定，该弧在两端实际等于水平直线段，而不只是具有相同切线。

令
\[
\mathcal C=\{(hU(s),hV(s)):0\le s\le1\}.
\]
则 \(\mathcal C\subset\{(u,z):u\ge0,\ |z|\le h\}\)，它从 \((0,h)\) 连到 \((0,-h)\)，两端有水平直线胚芽。模板只选一次，所以其曲率及任意阶弧长导数都有固定上界。
...
对每个整数 \(k\ge0\)，存在只依赖于 \(k,\chi,\mathcal C,h\) 的常数 \(C_k\)，使
\[
\sup_i\sup_{M_0^i}|\nabla^kA_i(0)|\le C_k.
\tag{2.5}
\]
